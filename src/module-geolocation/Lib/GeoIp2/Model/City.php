...
class City extends Country {
	...
	public function __construct($raw, $locales = ['en']) {
		...
		$this->city = new \Eloom\Geolocation\Lib\GeoIp2\Record\City($this->get('city'), $locales);
		$this->location = new \Eloom\Geolocation\Lib\GeoIp2\Record\Location($this->get('location'));
		$this->postal = new \Eloom\Geolocation\Lib\GeoIp2\Record\Postal($this->get('postal'));
	...
	private function createSubdivisions($raw, $locales) {
		...
		foreach ($raw['subdivisions'] as $sub) {
			array_push(
				$this->subdivisions,
				new \Eloom\Geolocation\Lib\GeoIp2\Record\Subdivision($sub, $locales)
			);
		}
	}
	...
	private function mostSpecificSubdivision() {
		return empty($this->subdivisions) ?
			new \Eloom\Geolocation\Lib\GeoIp2\Record\Subdivision([], $this->locales) :
